...
export async function generateInvoicePdf(invoice) {
  ...
  const customerLabel = invoice.customerCode
...
export async function generateBulkInvoicePdf(invoice) {
  const doc = new jsPDF({ orientation: "portrait", unit: "mm", format: "a4" });
  const pageW = doc.internal.pageSize.getWidth();
  const margin = 14;
  ...
  const tableRows = (invoice.lineItems || []).map((item) => {
    const description = item.isCustom
      ? item.customDescription || "Custom Service"
      : item.tierHours
        ? `${item.serviceType || "Service"} - ${item.tierHours} hours`
        : item.serviceType || "Service";
    const vehicleLabel = item.vehicleModelName
      ? `${item.vehicleModelName}${item.vehiclePlateNumber ? " - " + item.vehiclePlateNumber : ""}`
      : "—";
    return [
  ...
  autoTable(doc, {
    startY: y,
    head: [["Date", "Description", "Vehicle", "Amount"]],
    body: tableRows,
    margin: { left: margin, right: margin },
    styles: {
      fontSize: 8,
